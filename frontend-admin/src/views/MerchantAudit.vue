<template>
  <div class="merchant-audit">
    ...
    <div class="page-header">
      <h1>商户审核</h1>
      <p class="subtitle">审核商户入驻申请，管理商户状态</p>
    </div>
    ...
    <section class="filter-section">
      <div class="filter-bar">
        ...
        <div class="filter-actions">
          <button class="btn btn-primary" @click="loadMerchants">刷新列表</button>
        </div>
      </div>
    </section>
          ...
          >
            ...
            <div class="item-content">
              ...
              <div class="item-meta">
                ...
                <span>🕐 {{ formatTime(merchant.createdAt) }}</span>
              </div>
            </div>
            <div class="item-actions" @click.stop>
              ...
              <button 
                ...
                @click="openRejectModal(merchant.id)"
              >
                拒绝
              </button>
              <button class="btn btn-sm btn-ghost" @click="viewDetail(merchant)">
                详情
              </button>
            </div>
          </div>
    ...
    <div v-if="showDetailModal" class="modal-overlay" @click.self="closeDetailModal">
      <div class="modal-card detail-modal">
        ...
        <div class="modal-body">
          ...
          <div class="detail-row" v-if="detailData.remark">
...
// ========== Computed ==========
const pendingCount = computed(() => merchants.value.filter(m => m.status === 0).length);
const approvedCount = computed(() => merchants.value.filter(m => m.status === 1).length);
const rejectedCount = computed(() => merchants.value.filter(m => m.status === 2).length);

const filteredMerchants = computed(() => {
  if (filterStatus.value === "all") return merchants.value;
  return merchants.value.filter(m => m.status === Number(filterStatus.value));
});
...
const openRejectModal = (merchantId = null) => {
  rejectTargetId.value = merchantId;
  rejectReason.value = "";
...
const confirmReject = async () => {
  ...
  if (rejectTargetId.value) {
    // 单个拒绝
    const response = await rejectMerchant(rejectTargetId.value);
    if (response.success) {
      showToast('商户已拒绝');
      await loadMerchants();
    }
    ...
    showToast(`已拒绝 ${selectedIds.value.length} 个商户`);
    selectedIds.value = [];
    await loadMerchants();
  }
...
const batchApprove = async () => {
  ...
  showToast(`已通过 ${selectedIds.value.length} 个商户`);
  selectedIds.value = [];
  await loadMerchants();
};

const viewDetail = (merchant) => {
  detailData.value = { ...merchant };
...
const approveFromDetail = async () => {
  await approveMerchantAction(detailData.value.id);
  closeDetailModal();
};
